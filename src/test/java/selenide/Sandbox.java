...
public class Sandbox {
    ...
    void closeBasicAuth() {
        ...
        actions().sendKeys(Keys.ESCAPE); //Эскейп не сработал, но тест продолжился и выполнился))
        sleep(5000);
    ...
    void findAttributeByValue() {
        $("[name$=order]").click(); //кликаем на элемент, атрибут которого заканчивается на order
        $("[name^=order]").click(); //кликаем на элемент, атрибут которого начинается на order
    }

    @Test
    void required() {
        $("input#username").shouldHave(attribute("required"));// проверка наличия атрибута у элемента
    }
